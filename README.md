...
```
```bash echo "postgres" > secrets/postgres_password.txt ``` (Пароль можно заменить на любой. При первом запуске PostgreSQL инициализирует базу,
...
```bash echo "GitLabRoot123" > secrets/gitlab_root_password.txt ``` (пароль можно сменить, но значение берётся GitLab только при первом запуске, далее пароль хранится во внутренней БД GitLab.)
...
```bashSTORAGE=mongo docker compose up -d --build```
```bash
